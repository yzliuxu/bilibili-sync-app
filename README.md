...
这是一个自动化工具，旨在将 Bilibili 的视频（包括单个视频、合集、播放列表）高效地同步到云端存储（如 115 网盘）。它采用“下载一个、上传一个、即时销毁一个”的流水线模式，极大降低了对本地硬盘空间的依赖。
🌟 核心特性
   ...
   - 高性能后端：基于 FastAPI 开发，使用 SQLite WAL 模式确保读写并发性能，并针对 SSD/HDD 寿命进行了写入频率优化。
...
3. 前端部署
   ...
   npm run build
   ```
    构建产物 dist 目录将由后端 main.py 自动挂载，直接访问 http://localhost:8000 即可。
...
🛠️ 系统优化
    ...
    [ ] Cookies：目前 115 不支持使用 Cookies 自动组装成Rclone能读取的配置文件，需要手动组装。
